...
from snow_flask_core.validation import ValidationError, validate_address, validate_name

logger = logging.getLogger(__name__)


def create_app(template_folder: str | None = None, static_folder: str | None = None) -> Flask:
    ...
    def homepage() -> str | tuple[str, int]:
        ...
        try:
            ...
            return render_template(
                "error.html", error_title="Error", error_message="Failed to load data"
            ), 500
    ...
    @app.route("/HardData")
    def hard_data() -> str | tuple[str, int]:
        ...
        try:
            ...
            with get_db().cursor() as cur:
                cur.execute(
                    "SELECT ADDRESS, NAME FROM ADDRESSES LIMIT %s OFFSET %s", (limit, offset)
                )
    ...
    def thanks4submit() -> str | tuple[str, int]:
        ...
        try:
            ...
            with get_db().cursor() as cur:
                cur.execute("INSERT INTO ADDRESSES(ADDRESS, NAME) VALUES (%s, %s)", (address, name))

            logger.info("Address submitted", extra={"name_length": len(name)})
            return render_template("thanks4submit.html", colorname=address, username=name)
        except ValidationError as e:
            return render_template(
                "error.html", error_title="Invalid Input", error_message=str(e)
            ), 400
        except Exception:
            logger.error("Submission failed", exc_info=True)
            return render_template(
                "error.html", error_title="Error", error_message="Submission failed"
            ), 500
